...
from .z_image_transformer2d import (ADALN_EMBED_DIM, SEQ_MULTI_OF, FeedForward,
                                    RopeEmbedder, TimestepEmbedder)
...
class ZImageOmniTransformer2DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for layer_idx, layer in enumerate(self.layers):
                unified = self._gradient_checkpointing_func(
                    layer,
                    unified, unified_attn_mask, unified_freqs_cis, None,
